...
try{
 world=await createWorld(document.querySelector('#landscape'),p=>{document.querySelector('.progress i').style.width=`${Math.round(p*100)}%`;document.querySelector('.loading small').textContent=p>=1?'正在构建场景与交互…':`下载场景 ${Math.round(p*100)}%`;},reduced);
 if(new URLSearchParams(location.search).get('from')==='home'&&world.atlas){const destination=new URLSearchParams(location.search).get('region');document.querySelector('.loading small').textContent='正在准备所选城镇…';await world.atlas.readyFor(destination);if(world.atlas.regions.find(r=>r.id===destination)?.status==='error')throw Error('目标城镇未能加载');}
 window.__aether={ready:true,world,stats:()=>world.stats()};if(new URLSearchParams(location.search).get('region')==='highland')go(7);else if(new URLSearchParams(location.search).get('chapter')==='garden')go(3);document.querySelector('.loading').classList.add('done');setTimeout(()=>document.querySelector('.loading')?.remove(),900);
 const canvas=world.renderer.domElement;canvas.setAttribute('tabindex','0');canvas.setAttribute('aria-label','3D 群岛，方向键切换区域，空格暂停动画');
 if(world.atlas){world.visitor=createVisitorExperience(world);world.shell=createWorldShell({world});}
 if(world.atlas){const params=new URLSearchParams(location.search),destination=params.get('region'),quarter=params.get('quarter'),valleyTown=params.get('town'),forestGarden=params.get('conservatory'),starHall=params.get('observatory');world.atlas.readyFor(destination).then(()=>{if(destination&&(params.get('from')==='home'||!['highland','aether'].includes(destination)||params.get('walk')==='1'))world.atlas.travel(destination,params.get('arrival')==='portal'?'portal':'teleport');if(params.get('ferry')==='board')world.atlas.quarter.ferry?.board();if(quarter==='overview')world.atlas.quarter.overview();else if(['archive','inn','harbor'].includes(quarter))world.atlas.quarter.focus(quarter,0);if(valleyTown==='overview')world.atlas.valleyTown.overview();else if(['hall','lodge','storehouse'].includes(valleyTown))world.atlas.valleyTown.focus(valleyTown,0);if(forestGarden==='overview')world.atlas.forestGarden.overview();else if(['glasshouse','herbarium'].includes(forestGarden))world.atlas.forestGarden.focus(forestGarden,0);if(starHall==='hall')world.atlas.starHall.focus(true);else if(starHall==='overview')world.atlas.starHall.focus(false);else if(starHall==='telescope')world.atlas.starHall.instrument();});}
...
}catch(error){console.error(error);document.querySelector('.loading').innerHTML='<h2>群岛暂时没有展开</h2><p>请确认浏览器支持 WebGL，并重新载入模型。</p><button onclick="location.reload()">重新载入</button><a href="./index.html">前往紫境森林</a>';}
