...
document.querySelector('#app').innerHTML=`
 <main title="拖动环顾 · 滚轮探索 · 方向键切换区域" id="world" aria-label="三维水庭，可拖动环顾或滚动沿石桥探索"></main><div class="shade"></div>
 <header class="header"><button class="brand" aria-label="返回林间入口">ZIYU <span>/ 水庭</span></button><nav class="nav" aria-label="主导航"><button data-panel="world">世界</button><button data-panel="about">关于</button><button data-panel="works">作品</button><button class="sound" aria-label="开启环境音" aria-pressed="false">${speaker()}</button></nav></header>
 <section class="hero" aria-live="polite"><h1>在林深处，<br>留一处自己。</h1><p>一座可以走进的数字花园。</p><button class="cta" disabled>开始漫游 ${arrow}</button></section>
 <footer class="footer"><nav class="chapters" aria-label="场景章节">${chapters.map((c,i)=>`<button class="chapter ${i===0?'active':''}" data-chapter="${i}" aria-current="${i===0?'step':'false'}" disabled><strong>0${i+1}</strong><span>${c.name}</span></button>`).join('')}</nav><button class="play" disabled aria-label="${reduced?'播放动画':'暂停动画'}">${reduced?play:pause}<span>${reduced?'播放':'暂停'}</span></button></footer>
 <div class="loading" role="status"><svg class="gate" viewBox="0 0 48 70"><path d="M5 66V29Q5 14 24 3Q43 14 43 29V66M12 66V30Q12 20 24 12Q36 20 36 30V66"/></svg><p>正在走进水庭</p><div class="load-track"><i></i></div><span class="load-caption">正在载入水庭</span></div><div class="toast" role="status"></div>
...
function updateChapter(index,immediate=false){if(index===active&&!immediate)return;active=index;document.querySelector(`[data-chapter="${index}"]`)?.scrollIntoView({block:'nearest',inline:'nearest',behavior:reduced?'instant':'smooth'});document.querySelectorAll('.chapter').forEach((b,i)=>{b.classList.toggle('active',i===index);b.setAttribute('aria-current',i===index?'step':'false');});const apply=()=>{$('.hero h1').innerHTML=chapters[index].title.join('<br>');$('.hero h1').classList.toggle('long',index===3);$('.hero p').textContent=chapters[index].description;$('.hero').classList.remove('changing');};clearTimeout(heroTimer);if(immediate||reduced)apply();else{$('.hero').classList.add('changing');heroTimer=setTimeout(apply,240);}}
function go(index){if(!world)return;world.go(index);touring=false;$('.cta').innerHTML='开始漫游 '+arrow;updateChapter(index);}
function showPanel(type){const panel=$('#panel');if(type==='world'){if(panel.open)panel.close();go(0);return;}previousFocus=document.activeElement;
 if(type==='about')$('.panel-body').innerHTML=`<p class="panel-label">${profile.name} / 关于</p><h2 id="panel-title">${profile.title}</h2><p class="description">${profile.description}</p><p class="note">${profile.note}</p><button class="cta return-world">回到水庭 ${arrow}</button>`;
 else $('.panel-body').innerHTML=`<p class="panel-label">ZIYU / 作品</p><h2 id="panel-title">一些正在生长的想法。</h2>${projects.map(p=>`<button class="project" data-visit="${p.chapter}"><small>${p.type}</small><h3>${p.title}</h3><p>${p.detail}</p></button>`).join('')}<p class="note">以上为当前水庭中的场景实验。可在内容文件中替换为个人作品。</p>`;
...
$('.cta').addEventListener('click',()=>{if(!world)return;touring=!touring;world.setTouring(touring);if(touring){playing=true;updatePlay();}$('.cta').innerHTML=(touring?'停止漫游':'开始漫游')+' '+arrow;});
document.querySelectorAll('[data-chapter]').forEach(b=>b.addEventListener('click',()=>go(Number(b.dataset.chapter))));
window.addEventListener('journeychange',e=>{touring=false;$('.cta').innerHTML='开始漫游 '+arrow;updateChapter(Math.round(e.detail*(chapters.length-1)));});window.addEventListener('tourend',()=>{touring=false;$('.cta').innerHTML='再次漫游 '+arrow;updateChapter(chapters.length-1);});
window.addEventListener('keydown',e=>{if($('#panel').open||['INPUT','TEXTAREA','BUTTON'].includes(document.activeElement?.tagName))return;if(e.code==='Space'){e.preventDefault();$('.play').click();}else if(e.key==='ArrowRight'||e.key==='ArrowDown'){e.preventDefault();go(Math.min(chapters.length-1,active+1));}else if(e.key==='ArrowLeft'||e.key==='ArrowUp'){e.preventDefault();go(Math.max(0,active-1));}else if(e.key==='Home'){go(0);}});
document.addEventListener('visibilitychange',()=>{document.hidden?ambience.suspend():ambience.resume();});
try{world=await createWorld($('#world'),p=>{$('.load-track i').style.width=Math.round(p*100)+'%';$('.load-caption').textContent='水庭载入 '+Math.round(p*100)+'%';},reduced);$('.loading').classList.add('done');document.querySelectorAll('button:disabled').forEach(b=>b.disabled=false);setTimeout(()=>$('.loading').remove(),1000);window.__watercourt={world,go,stats:()=>world.stats(),ready:true};setInterval(()=>{if(touring)updateChapter(Math.min(chapters.length-1,Math.round(world.getProgress()*(chapters.length-1))));},500);world.portal.ready();rendererContextRecovery();}
catch(error){console.error(error);$('.loading').remove();$('#world').style.background='center / cover url(./watercourt/reference.png)';const box=document.createElement('div');box.className='error';box.setAttribute('role','alert');box.innerHTML='三维场景暂时无法加载，当前展示参考图。请使用支持 WebGL 的浏览器。<button>重新加载</button>';box.querySelector('button').onclick=()=>location.reload();$('#app').appendChild(box);}
