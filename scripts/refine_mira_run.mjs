...
const input=fs.readFileSync(path.join(root,'outputs/character/meshy-mira/mira-balanced-walk-v4.glb'));
...
const neutral={hip:worldP(bones.Hips),feet:{},footRotations:{},hipRotation:worldQ(bones.Hips),toeRotations:Object.fromEntries(["Left","Right"].map(side=>[side,bones[side+"ToeBase"].quaternion.clone()]))};
...
const report={source:'mira-balanced-walk-v4.glb',strideScale:.72,ankleTrackHalfWidth:.08,armSwingScale:.55,pairedLegCycle:true,clips:{}};
for(const name of ['Run']){
 ...
 // Remove outward foot roll/yaw from the running delta, retaining boot pitch.
 const pitchOnly=q=>{const n=Math.hypot(q.x,q.w);return new T.Quaternion().slerp(n>1e-6?new T.Quaternion(q.x/n,0,0,q.w/n):new T.Quaternion(),.72);};
 ...
 for(let i=0;i<=64;i++){
  ...
  // Keep the leather boot forefoot in its neutral local shape. The source
  // toe curl twists the two soles differently during push-off.
  for(const side of ['Left','Right'])bones[side+'ToeBase'].quaternion.copy(neutral.toeRotations[side]);
  if(name==='Run'){
   nodes.forEach((o,k)=>{let amplitude;if(/Shoulder|Arm|Hand/.test(o.name))amplitude=/ForeArm/.test(o.name)?.65:.55;else if(/Spine|Head|Neck/.test(o.name))amplitude=.65;if(amplitude!==undefined)o.quaternion.copy(mean[k]).slerp(sample.q[k],amplitude);});
   const half=samples[(i+32)%64];
   const hp=neutral.hip.clone();hp.y+=((sample.hip.y+half.hip.y)*.5-neutral.hip.y)*.55;
   ...
   setWorldRotation(bones.Hips,new T.Quaternion().slerp(delta.slerp(paired,.5),.35).multiply(neutral.hipRotation));
  ...
  for(const side of ['Left','Right']){
   ...
   if(name==='Run'){
    ...
    target.x=neutral.hip.x+sign*.08;
    target.z=(neutral.feet.Left.z+neutral.feet.Right.z)*.5+cycle.z*.72;
    target.y=rest.y+cycle.y*.70;
  ...
  if(name==='Run')lowerPelvisForReach(targets);
...
// Apply the measured boot contact profile: shorter, lower flight phases.
// Walk and Idle remain byte-for-byte unchanged from V4.
const contactPath=path.join(root,'scripts/data/mira-run-contact.json');
if(process.argv.includes('--contact')){
 ...
 for(const name of ['Run']){const clip=doc.animations.find(c=>c.name===name),ch=clip.channels.find(c=>doc.nodes[c.target.node].name==='Hips'&&c.target.path==='translation'),sp=clip.samplers[ch.sampler],times=stagedRead(sp.input),values=stagedRead(sp.output);contact[name].forEach((sample,i)=>values[i*3+1]+=sample.lift*100);sp.output=append(values,'VEC3');}
 report.contactCorrected=true;
}
doc.buffers[0].byteLength=bytes;doc.asset.extras={...doc.asset.extras,gaitRevision:'Centered symmetric running tracks, reduced heel lift and foot roll; preserves V4 Walk and Idle.'};let json=Buffer.from(JSON.stringify(doc));json=Buffer.concat([json,Buffer.alloc(-json.length&3,32)]);let bin=Buffer.concat(chunks);bin=Buffer.concat([bin,Buffer.alloc(-bin.length&3)]);const head=Buffer.alloc(20);head.write('glTF');head.writeUInt32LE(2,4);head.writeUInt32LE(28+json.length+bin.length,8);head.writeUInt32LE(json.length,12);head.writeUInt32LE(0x4e4f534a,16);const bh=Buffer.alloc(8);bh.writeUInt32LE(bin.length);bh.writeUInt32LE(0x004e4942,4);const result=Buffer.concat([head,json,bh,bin]);
for(const name of ['public/character/mira-grounded-run-v5.glb','outputs/character/meshy-mira/mira-grounded-run-v5.glb'])fs.writeFileSync(path.join(root,name),result);
fs.mkdirSync(path.join(root,'work/meshy-mira/gait-run'),{recursive:true});fs.writeFileSync(path.join(root,'work/meshy-mira/gait-run/metrics.json'),JSON.stringify(report,null,2));console.log(report);
