...
const root=process.cwd(),out=root+'/outputs/aether',port=process.env.AETHER_PORT||'5176';
...
try{
 ...
 if(!stats.floatingIslands||stats.waterfalls!==5||errors.length)throw Error(JSON.stringify({stats,errors}));
 ...
 const fps=24,seconds=30,frames=fps*seconds;
 const encoder=spawn(ffmpeg,['-y','-f','image2pipe','-vcodec','png','-r',String(fps),'-i','pipe:0','-an','-c:v','libx264','-preset','fast','-crf','19','-pix_fmt','yuv420p','-movflags','+faststart',out+'/aether-journey.mp4'],{stdio:['pipe','ignore','pipe']});
 ...
 for(let frame=0;frame<frames;frame++){
  ...
  if(frame%120===0)await writeFile(out+`/scene-${String(frame/120+1).padStart(2,'0')}.png`,bytes);
