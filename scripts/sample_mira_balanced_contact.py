# Run after balance_mira_walk.mjs WITHOUT --contact to sample the raw gait.
...
bpy.ops.import_scene.gltf(filepath=str(root/'public/character/mira-balanced-walk-v4.glb'))
...
(root/'scripts/data/mira-balanced-contact.json').write_text(json.dumps(result,indent=2));print('CONTACT_SAMPLES',[(k,len(v),min(x['sole'] for x in v),max(x['sole'] for x in v)) for k,v in result.items()])
